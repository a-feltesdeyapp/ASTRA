...
double Kp = 1.20; //proportional gain
...
float startingPos; // initial encoder position
//SET DESIRED MOVEMENT HERE
float desiredMovement = -45; // desired motor movement in degrees + for counterclockwise, - for clockwise
float pwm;
float min_pwm=24;
...
void setup(){
  ...
  // establish target angle
  readEncoder();
  startingPos = encoderPosition * degreesPerCount;
  theta_t = startingPos + desiredMovement;
  theta_t = fmod(theta_t,360);
...
void loop(){
readEncoder();
theta = encoderPosition * degreesPerCount;
